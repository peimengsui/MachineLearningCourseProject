...
def train_and_score(_tr, _va, model_size):
	...
	model = SGDClassifier(loss="hinge", penalty="l2", n_jobs=-1)
	...
	joblib.dump(model, 'model_svm_l2_size%d.pkl' % model_size)
	model = joblib.load('model_svm_l2_size%d.pkl' % model_size)
	...
	np.save("labels_svm_size%d.npy" % model_size, all_labels)
	np.save("preds_svm_size%d.npy" % model_size, all_preds)
	'''
	all_labels = np.load("labels_svm_size%d.npy" % model_size)
	all_preds = np.load("preds_svm_size%d.npy" % model_size)
	print "Scoring"
	...
	plt.tight_layout()
	plt.savefig('model_svm_l2_size%d.png' % model_size)
	plt.show()
